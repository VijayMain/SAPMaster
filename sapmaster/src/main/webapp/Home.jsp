...
<hr /> 
 ...
 <tr style='font-size: 14px; background-color: #e9d2be; border-width: 1px; padding: 8px; border-style: solid;border-color: #729ea5;text-align: center;height: 25px;'>
	<th onclick="buttonCSTMst('cust');" style="cursor: pointer;background-color: #793800;color: white;">Customer / Vendor Master List</th>
...
String masterLogin="",user_Req="";
int s_no=1;
PreparedStatement ps_check = null, ps_upload = null, ps_apset = null, ps_mst2=null;
ResultSet rs_check = null, rs_upload = null, rs_apset=null,rs_mst2=null;
  ...
  PreparedStatement	ps_check1 = con.prepareStatement("SELECT  * FROM user_rel where old_id="+ userId);
	...
	if(masterLogin.equalsIgnoreCase("APPROVER")){
%>
<form action="Approval.jsp" method="post" name="approve" id="approve">
  ...
  <%-- <%
	...
	while (rs_check.next()) {
		 ps_mst2 = con1.prepareStatement("SELECT * FROM user_tbl where u_id="+ Integer.parseInt(rs_check.getString("uid")));
		 rs_mst2 = ps_mst2.executeQuery();
			while (rs_mst2.next()) {
				user_Req=rs_mst2.getString("U_Name");
			}
 %>						
				<tr onmouseover="ChangeColor(this, true);" onmouseout="ChangeColor(this, false);" onclick="button1('<%= rs_check.getString("id") %>');" style="cursor: pointer;">
	...
	%>		 --%>		
	<%
  	ps_check = con.prepareStatement("SELECT * FROM sap_masterVendCust where enable=1");
	rs_check = ps_check.executeQuery();
	while (rs_check.next()) {
		
		ps_upload = con.prepareStatement("SELECT * FROM approval_VendCust_rel where id_approval>1 and id_vendormaster="+rs_check.getInt("id"));
		rs_upload = ps_upload.executeQuery();
		if(rs_upload.next()){
...
<%
	}else if(masterLogin.equalsIgnoreCase("MAT_CRTD_TEAM")){
%>
		<form action="DownloadMaster.jsp" method="post" name="masterUp" id="masterUp">
		  <!-- <input type="hidden" name="hid" id="hid"/> -->
		  <table border='1' width='100%'>
		<tr style='font-size: 12px; background-color: #acc8cc; border-width: 1px; padding: 8px; border-style: solid;border-color: #729ea5;text-align: center;'>
			...
			<input type="hidden" name="hidMst" id="hidMst"/>  
		  <%
		  	ps_check = con.prepareStatement("SELECT * FROM sap_masterVendCust where stage not in(2,4) and  id IN (SELECT id_vendormaster FROM approval_VendCust_rel where id_approval=3)");
			rs_check = ps_check.executeQuery();
			while (rs_check.next()) {
		 ...
		 %>
						<tr onmouseover="ChangeColor(this, true);" onmouseout="ChangeColor(this, false);" onclick="button_mst('<%= rs_check.getString("id") %>');" style="cursor: pointer;">
									<td style='text-align: center;'> <%= s_no %></td>
									<td> <%=rs_check.getString("cust_vend")%></td>
									<td> <%=rs_check.getString("name_org")%></td> 
									<td> <%=rs_check.getString("reason_for")%></td>
			...
			%>
			<%
		  	ps_check = con.prepareStatement("SELECT * FROM sap_masterVendCust where stage=2 and  id IN (SELECT id_vendormaster FROM approval_VendCust_rel where id_approval=3)");
			rs_check = ps_check.executeQuery();
			while (rs_check.next()) {
				 ps_mst2 = con1.prepareStatement("SELECT * FROM complaintzilla.user_tbl where u_id="+ Integer.parseInt(rs_check.getString("uid")));
				 rs_mst2 = ps_mst2.executeQuery();
		 ...
		 %>
						<tr onclick="button_mst('<%= rs_check.getString("id") %>');" style="height:8px; cursor: pointer;">
									<td style='text-align: center;'> <%= s_no %></td>
									<td> <%=rs_check.getString("cust_vend")%></td>
									<td> <%=rs_check.getString("name_org")%></td> 
									<td> <%=rs_check.getString("reason_for")%></td>
									...
									<td> <%=rs_check.getString("email")%></td> 
									<td> <%=rs_check.getString("mobile_no")%></td> 
									<td style="background-color: #9eff78"><b>Done</b></td></tr>
			<%
				s_no ++;
			user_Req="";
				}			
			...
			<%
		  	ps_check = con.prepareStatement("SELECT * FROM sap_masterVendCust where stage=4 and  id IN (SELECT id_vendormaster FROM approval_VendCust_rel where id_approval=3)");
			rs_check = ps_check.executeQuery();
			while (rs_check.next()) {
				 ps_mst2 = con1.prepareStatement("SELECT * FROM complaintzilla.user_tbl where u_id="+ Integer.parseInt(rs_check.getString("uid")));
				 rs_mst2 = ps_mst2.executeQuery();
		 ...
		 %>
						<tr onclick="button_mst('<%= rs_check.getString("id") %>');" style="height:8px; cursor: pointer;">
									...
									<td> <%=rs_check.getString("email")%></td> 
									<td> <%=rs_check.getString("mobile_no")%></td> 
									<td  style="background-color: #ff2828"><b>Rejected</b></td></tr>
			<%
				s_no ++;
			user_Req="";
				}			
...
<%
	}else if(!masterLogin.equalsIgnoreCase("MAT_CRTD_TEAM") && !masterLogin.equalsIgnoreCase("APPROVER")){
%>
<form action="All_Registrations.jsp" method="post" name="masterReg" id="masterReg">
		  ...
		  <% 
		  String apr_set="";
		  	ps_check = con.prepareStatement("SELECT * FROM sap_masterVendCust where stage!=4 and uid ="+userId+" and id IN (SELECT id_vendormaster FROM approval_VendCust_rel)");
			rs_check = ps_check.executeQuery();
			while (rs_check.next()) {
		 ...
		 %>
						<tr onmouseover="ChangeColor(this, true);" onmouseout="ChangeColor(this, false);" onclick="button_mstReg('<%= rs_check.getString("id") %>');" style="cursor: pointer;">
									...
									<td> <%=rs_check.getString("email")%></td> 
									<td> <%=rs_check.getString("mobile_no")%></td> 
									<td style="color : red;width: 100px;"><b>Rejected <br /><%= rs_check.getString("reject_reason")%>..Rejected By <%= rs_check.getString("rejected_by")%></b></td></tr>
			<%
				s_no ++;
				}
			user_Req="";
